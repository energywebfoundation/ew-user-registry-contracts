    ...
    export class UserLogic extends GeneralFunctions
{
    ...
    constructor(web3: Web3, address?: string){
        super(address ? new web3.eth.Contract(UserLogicJSON.abi, address) : new web3.eth.Contract(UserLogicJSON.abi, UserLogicJSON.networks.length > 0 ? (UserLogicJSON.networks[0]) : null))
        this.web3 = web3
    }
    ...
    async getAllEvents(eventFilter?:SearchLog){
    	...
    	async deactivateUser(_user:string, txParams?:SpecialTx){
            ...
            const txData = await this.web3Contract.methods.deactivateUser(_user)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.deactivateUser(_user)
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.deactivateUser(_user)
	...
	async setRoles(_user:string,_rights:number, txParams?:SpecialTx){
            ...
            const txData = await this.web3Contract.methods.setRoles(_user,_rights)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.setRoles(_user,_rights)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.setRoles(_user,_rights)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
