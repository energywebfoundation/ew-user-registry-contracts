    ...
    export class UserContractLookup extends GeneralFunctions
{
    ...
    constructor(web3: Web3, address?: string){
        super(address ? new web3.eth.Contract(UserContractLookupJSON.abi, address) : new web3.eth.Contract(UserContractLookupJSON.abi, UserContractLookupJSON.networks.length > 0 ? (UserContractLookupJSON.networks[0]) : null))
        this.web3 = web3
    }
    ...
    async getAllEvents(eventFilter?:SearchLog){
    	...
    	async update(_userRegistry:string, txParams?:SpecialTx){
            ...
            const txData = await this.web3Contract.methods.update(_userRegistry)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.update(_userRegistry)
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.update(_userRegistry)
	...
	async init(_userRegistry:string,_db:string, txParams?:SpecialTx){
            ...
            const txData = await this.web3Contract.methods.init(_userRegistry,_db)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.init(_userRegistry,_db)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.init(_userRegistry,_db)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
