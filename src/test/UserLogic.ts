...
describe("UserLogic", () => {
  ...
  const accountDeployment = web3.eth.accounts.privateKeyToAccount(
  ...
  it("should deploy the contracts", async () => {
    ...
    Object.keys(contracts).forEach(async key => {
      ...
      if (key.includes("UserContractLookup")) {
        userContractLookup = new UserContractLookup(
          web3 as any,
          contracts[key]
        );
  ...
  it("should throw an error when calling init again", async () => {
    ...
    try {
      await userLogic.init(
        userLogic.web3Contract._address,
        userLogic.web3Contract._address,
  ...
  it("should return 0 rights for random accounts", async () => {
    assert.equal(
      await userLogic.getRolesRights(
  ...
  it("should fail trying to set roles as non user-Admin", async () => {
    ...
    try {
      await userLogic.setRoles(
        ...
        {
          privateKey:
            "0x191c4b074672d9eda0ce576cfac79e44e320ffef5e3aadd55e000de57341d36c"
        }
      );
    } catch (ex) {
      failed = true;
  ...
  it("should fail when trying to deactive an active admin-account", async () => {
    let failed = false;
    try {
      await userLogic.deactivateUser(
        "0x1000000000000000000000000000000000000005",
        { privateKey: privateKeyDeployment }
      );
    } catch (ex) {
