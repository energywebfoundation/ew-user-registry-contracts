...
const fs = require("fs");
class Owned extends GeneralFunctions_1.GeneralFunctions {
    constructor(web3, address) {
        super(address ? new web3.eth.Contract(JSON.parse(fs.readFileSync('dist/contracts/Owned.json', 'utf8').toString()).abi, address) : new web3.eth.Contract(JSON.parse(fs.readFileSync('dist/contracts/Owned.json', 'utf8').toString()).abi, Object.keys(JSON.parse(fs.readFileSync('dist/contracts/Owned.json', 'utf8').toString()).networks).length > 0 ? JSON.parse(fs.readFileSync('dist/contracts/Owned.json', 'utf8').toString()).networks[Object.keys(JSON.parse(fs.readFileSync('dist/contracts/Owned.json', 'utf8').toString()).networks)[0]].address : null));
        this.buildFile = JSON.parse(fs.readFileSync('dist/contracts/Owned.json', 'utf8').toString());
        this.web3 = web3;
    }
